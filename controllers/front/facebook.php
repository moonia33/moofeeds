...
class moofeedsfacebookModuleFrontController extends ModuleFrontController
{
    private function cacheBase(){ return _PS_MODULE_DIR_ . 'moofeeds/var/cache/'; }
    private function tryServeCache($langId,$shopId,$currencyIso){ $base=$this->cacheBase(); $file=sprintf('%s%s-%d-%d-%s.csv',$base,'facebook',$shopId,$langId,$currencyIso); if(is_file($file)){ header('Content-Type: text/csv; charset=UTF-8'); header('X-Content-Type-Options: nosniff'); $mtime=filemtime($file); header('Last-Modified: '.gmdate('D, d M Y H:i:s',$mtime).' GMT'); $etag='W/"'.md5($file.$mtime.filesize($file)).'"'; header('ETag: '.$etag); header('Cache-Control: public, max-age=1800'); if((isset($_SERVER['HTTP_IF_NONE_MATCH']) && trim($_SERVER['HTTP_IF_NONE_MATCH'])===$etag)){ http_response_code(304); exit; } readfile($file); exit; } }
